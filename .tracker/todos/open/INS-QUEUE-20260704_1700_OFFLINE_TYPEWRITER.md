...
3. **Service Worker Interception (`sw.js`):** 
   - `GET` requests (e.g., `/api/citations`, `/api/system/config`) use a *Network First, falling back to IndexedDB* strategy.
...
9. **The CDN/ESM Trap (Asset Availability):** The Service Worker must implement a `Stale-While-Revalidate` caching strategy for cross-origin dependencies (e.g., `esm.sh` and `cdn.jsdelivr.net`). This guarantees that dynamic imports for CodeMirror, Lit, and Zustand do not crash the UI when the network is absent.
