...
function isClosed(doc, elt, name2) {
  var _a;
  for (; ; ) {
    if (((_a = elt.lastChild) === null || _a === void 0 ? void 0 : _a.name) != "CloseTag")
      return false;
    let next = elt.parent;
    if (!next || elementName(doc, next) != name2)
      return true;
    elt = next;
  }
}
var autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, insertTransaction) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    if (didType && text == ">" && after.name == "EndTag") {
      ...
      if ((name2 = elementName(state.doc, tag.parent, head)) && !selfClosers2.has(name2) && !isClosed(state.doc, tag.parent, name2)) {
      ...
      let tag = after.parent;
      if (after.from == head - 2 && ((_a = tag.lastChild) === null || _a === void 0 ? void 0 : _a.name) != "CloseTag" && (name2 = elementName(state.doc, tag, head)) && !selfClosers2.has(name2)) {
        let to2 = head + (state.doc.sliceString(head, head + 1) === ">" ? 1 : 0);
        let insert = `${name2}>`;
        return {
