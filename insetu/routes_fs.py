...
@fs_bp.route('/api/fs/archive', methods=['POST'])
def api_fs_archive():
    data = request.json
    filepath = data.get("filepath", "").strip()
...
@fs_bp.route('/api/fs/delete', methods=['POST'])
def api_fs_delete():
    data = request.json
    filepath = data.get("filepath", "").strip()
    if not filepath: return jsonify({"error": "Filepath required"}), 400
    ...
    return jsonify({"status": "success"})
@fs_bp.route('/api/fs/search', methods=['GET'])
def api_fs_search():
...
@fs_bp.route('/api/fs/compile-document', methods=['POST'])
...
def execute_vfs_save(workspace_id, filepath, content, data=None):
    """Core VFS processing logic decoupled from HTTP request frameworks."""
    if data is None:
        data = {}
    ...
    if filepath.lower().endswith('.json'):
        import json
        json.loads(content) # Let exception bubble up naturally to handler
    ...
    hooks.emit('post_file_save', filepath=filepath)
    return {"status": "success", "message": f"Saved {filepath}", "is_new": is_new}

@fs_bp.route('/api/fs/save', methods=['POST'])
def api_fs_save():
    """Universal save-back endpoint with explicit header routing guardrails."""
