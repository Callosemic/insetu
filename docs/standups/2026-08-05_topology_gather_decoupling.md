...
To eliminate async racing, execution follows a strict chronological waterfall:
1.  **Disk Mutation:** User saves a file -> Core OS emits `vfs_mutated`.
2.  **Topology Mapping:** Topology engine catches `vfs_mutated`, evaluates rules, assigns the bucket, UPSERTs SQLite, and emits `topology_resolved` (passing the dirty bucket IDs).
3.  **Context Compilation:** Gather listens *only* to `topology_resolved`. It queries SQLite for the dirty buckets, builds the `ctx://` artifacts, and dispatches the final unified manifest to the frontend.
...
4.  **Frontend Update:** Refactor `<insetu-file-tree>` to render physically tracked files from `manifest.vfs` and virtual/context payloads from `manifest.ctx`.
...
*   **Favorites Bar:** Existing database entries will require a simple migration script to prepend `vfs://` to physical paths and `ctx://` to context paths so the router parses them correctly.
