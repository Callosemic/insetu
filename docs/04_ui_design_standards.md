...
* **The Modal Scrolling Guardrail**: The `<yenvui-modal>` and `<sutram-modal>` primitives natively manage their own internal `overflow-y: auto` momentum scrolling. **Never** apply `overflow-y: auto`, `flex: 1`, or `min-height: 0` to elements projected into `slot="body"`. Doing so creates an inset "box within a box" double-scrollbar. If you project a wrapper into the body slot, use `display: contents;` or simple flex layouts without scroll constraints.
